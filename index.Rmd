---
title: "GF-0604<br> Procesamiento de datos geográficos<br>Laboratorio 03: R - Datos vectoriales 1"
author: "Aaron Blanco"
date: "2020-05-11"
output:
  html_document:
...
```{r}
ggplot(centro_america, aes(x=reorder (name_long, -lifeExp), y=lifeExp)) + ggtitle("Esperanza de vida países centroamericanos") + xlab("Países") + ylab("Esperanza de vida") + geom_col( color="blue") + geom_text(aes(label=lifeExp), vjust=-0.5, size=4)
```
...
```{r}
plot(centro_america['lifeExp'], main='Esperanza de vida países centroamericanos')
```
...
```{r}
centroamerica_74 <- centro_america[centro_america$lifeExp >= 74, ]
plot(centroamerica_74["lifeExp"], main = 'Esperanza de vida mayor o igual a 74 años en países centroamericanos')
```

### Extra
Uso de GIT por medio de la terminal para efectura los cambios en el repositorio de GitHub.
![Uso terminal](https://github.com/Aaronblancod/laboratorio-03-r-datos-vectoriales-1/blob/master/Uso%20terminal.png?raw=true)
